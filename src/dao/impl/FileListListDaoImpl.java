...
public class FileListListDaoImpl extends BaseDao implements FileListDao {
    @Override
    ...
    public boolean deleteFile(String uuid, String filename) {
        String sql_register = "delete from FileList where uuid=? and filename=?";
        Object[] obj_register = {uuid, filename};
        int result = this.modifyDRMSData(sql_register, obj_register);
        boolean flag = false;
        if(result>0) {
            flag = true;
    ...
    public List<FileList> showFile() {
        ...
        try {
            while (rs.next()) {
                try {
                    FileList file = new FileList(rs.getString("uuid"),rs.getString("filename"),rs.getString("filesize"),rs.getString("uploader"),rs.getString("uptime"));
                    filelist.add(file);
